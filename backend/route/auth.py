...
def loginAccessToken(
    ...
    access_token = createAccessToken(
        ...
        expires_delta=timedelta(minutes=60),  # เวลาหมดอายุของ Token
    )

    # สร้าง Refresh Token
    refresh_token = createRefreshToken(
        user_id=userAuth.user_id,
        token_version=userAuth.token_version,  # ส่งเลข Version ไปใน Refresh Token ด้วย
    )

    userAuth.refresh_token = refresh_token  # อัปเดต Refresh Token ในฐานข้อมูล
...
async def refreshToken(request: RefreshTokenRequest, db: Session = Depends(get_db)):
    try:
        ...
        # เช็คว่า Refresh Token กับ Version ถูกต้องไหม
        if (
            db_user is None
            or db_user.refresh_token != request.refresh_token
            or db_user.token_version != token_version
        ):
            raise HTTPException(
                status_code=401, detail="Refresh token invalid or revoked"
            )
